...
Provider-backed adoption uses the reusable `ai-skills/.github/workflows/consumer-acceptance.yml` pinned by full SHA **outside the assessed candidate**. That external workflow supplies the authority repository/revision independently, verifies equality with the candidate lock, checks real GitHub provider controls, validates exact-SHA evidence, and requires an independent review. Until the repository's branch/environment administration permits that path, structural CI results remain diagnostic rather than an `adopted` decision.
