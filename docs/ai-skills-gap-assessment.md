...
Provider-backed approval must instead be anchored outside the candidate by a full-SHA reference to the authority repository's reusable `consumer-acceptance.yml`. That workflow independently supplies the authority coordinates, compares them with the candidate lock, verifies real GitHub provider controls, validates exact-SHA provider evidence, and requires independent review.
...
- An externally pinned provider-backed adoption assessment and independent review bound to the final immutable candidate SHA are still missing. No reviewer or approval evidence is fabricated.
...
Keep PR #7 draft until repository-owned quality, latest-reviewed structural standards, Python compatibility, exact-artifact, official stdio/HTTP smoke, and security gates all pass on one immutable implementation SHA. Then complete the remaining live evidence that supports claimed real-backend behavior. Separately, administrators must establish observable provider controls and the externally pinned `ai-skills` reusable acceptance workflow must validate the same candidate SHA, its provider evidence, and an independent review. Only then may the migration state advance through provider validation/review to `adopted`; repository-owned green CI by itself is not that decision.
