...
- Authentication and authorization are distinct. Every invocation binds principal, exact capability, immutable target, explicit resource identity, and normalized-argument digest, then revalidates immediately before I/O. ID-bound capabilities reject missing, boolean, floating-point, zero, negative, or string IDs before authorization can claim an exact resource.
...
- The exact-artifact lane verifies the independently pinned `ai-skills` authority, captures the MCP public contract from the same installed wheel that is tested and smoke-tested, and includes the trusted `MCP_PUBLIC_CONTRACT.json` snapshot in the same `RELEASE_SHA256SUMS` closure and uploaded release bundle. Contract evidence is therefore bound to the exact release wheel rather than a separately rebuilt wheel.
- `trusted-executable-sources.lock.yaml` is the single authority coordinate and binds every trusted executable used by acceptance to a SHA-256 digest, including the MCP public-contract capture/comparison entrypoints. The trusted validator checks that lock before the other standards tools execute, and consumer-trust hygiene detects candidate-controlled trust bypasses.
...
- The exact historical `v1.0.1` public-contract baseline cannot currently be captured as authoritative artifact evidence: that GitHub release published no immutable wheel asset and the 1.0.1 package declared ranged dependencies. Rebuilding the tag today would resolve a different dependency graph and would fabricate historical exact-artifact evidence. The 2.0.0 major bump is deliberately conservative, and the exact candidate snapshot is captured for future comparisons, but a trusted `compare_mcp_contracts.py --check` baseline comparison remains unavailable unless an independently verifiable 1.0.1 deployment artifact is recovered.
