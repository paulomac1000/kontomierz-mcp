...
- A dedicated contract-evidence lane builds and installs the exact candidate wheel, verifies the independently pinned `ai-skills` authority, probes the server through the official MCP stdio client, and passes the observed document through the trusted `capture_mcp_contract.py` validator before retaining the canonical candidate snapshot as CI evidence.
