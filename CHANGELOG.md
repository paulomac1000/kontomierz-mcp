...
- Verified empty-body creates (wallet, budget, schedule) now return `{"created": true, "reconciliation_required": true}` on the confirmed 2xx instead of raising `AMBIGUOUS_OUTCOME`: the upstream confirms creation but never returns identity, and the usage notes already documented this marker contract. Timeouts and transport losses after start remain `AMBIGUOUS_OUTCOME`, and `create_transaction` stays fail-closed until live evidence exists. `mark_schedule_paid`/`mark_schedule_unpaid` parameter notes state that `payment_date` must equal the exact scheduled occurrence date (the upstream rejects other dates, including today, with 422).
