...
PRIMARY_NAMES = {
    "list_accounts", "create_wallet", "update_wallet", "destroy_wallet",
    "list_transactions", "get_transaction", "create_transaction",
    "update_transaction", "delete_transaction", "list_categories",
    "list_tags", "list_currencies",
}
...
async def dispatch_primary(name: str, a: dict[str, Any], client: Any) -> Any:
    ...
    if name == "create_wallet":
        ...
        return await resolve(client.create_wallet(
            money(a["currency_balance"], "currency_balance", positive=False),
            currency(a["currency_name"]), a.get("user_name"), liquid,
        ))
    ...
    if name == "list_transactions":
        ...
        items = await resolve(client.get_money_transactions(
            page=number, per_page=limit,
            user_account_id=identifier(a.get("user_account_id"), "user_account_id", optional=True),
            q=str(a.get("q", "")).strip() or None, start_on=start, end_on=end,
    ...
    if name == "create_transaction":
        fields: dict[str, Any] = {
            ...
            "tag_string": a.get("tag_string", ""), "name": a.get("name", ""),
        }
    ...
    if name == "update_transaction":
        fields = provided(a, (
            "user_account_id", "category_id", "currency_amount", "currency_name",
            "direction", "tag_string", "name", "transaction_on",
        ))
